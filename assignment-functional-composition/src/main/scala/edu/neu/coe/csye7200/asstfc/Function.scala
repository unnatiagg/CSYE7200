...
object Function {
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] =  map2(_,_)(f)
    // this actually converts to the same
    //(x,y) => for (x1 <- x; y1 <- y) yield f(x1,y1)
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] = (x,y,z) => for (x1 <- x; y1 <- y; z1 <- z) yield f(x1,y1, z1)
  // Can also be mentioned as following map3(_,_,_)(f)
  /**
  ...
  (Try[T1], Try[T2], Try[T3], Try[T4], Try[T5], Try[T6], Try[T7]) => Try[R] = {
    (x1,x2,x3,x4, x5, x6, x7) => for {
      ...
      a5 <- x5;
      a6 <- x6;
      a7 <- x7 } yield f(a1,a2,a3,a4,a5,a6,a7)
  }
  //can also be mentioned as  map7(_,_,_,_,_,_,_)(f), but this eventually evaluates to above
  /**
   * You get the idea...
   */
